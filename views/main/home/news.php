<!-- tin nông trại -->
<div class="content__container container">
    <h3 class="content__header">
        <i class="content__header-icon ri-file-list-3-line"></i>
        Tin nông trại
    </h3>

    <div class="content__item-list row">

        <div class="col-12 col-sm-6 col-md-4 col-xl-3">
            <a href="#" class="news card">
                <img src="<?php echo $imglink['news-evfta']; ?>" class="news__img">
                <div class="news__body">
                    <h5 class="news__title">Nhiều nông sản xuất khẩu tận dụng tốt EVFTA</h5>
                    <p class="news__brief">Theo Bộ Công thương, trong 6 tháng đầu năm 2022, kim ngạch xuất khẩu
                        của VN vào thị trường EU đạt gần 24 tỉ USD, tăng 22,6% so với năm 2021</p>
                </div>
            </a>
        </div>

        <div class="col-12 col-sm-6 col-md-4 col-xl-3">
            <a href="#" class="news card">
                <img src="<?php echo $imglink['news-rau']; ?>" class="news__img">
                <div class="news__body">
                    <h5 class="news__title">Vợ chồng bỏ việc tại viện nghiên cứu về trồng rau sạch</h5>
                    <p class="news__brief">Từng là nghiên cứu viên về lĩnh vực nông nghiệp, đôi vợ chồng ở Quảng
                        Bình đã bỏ việc về quê để gầy dựng thương hiệu nông sản sạch bằng khu vườn rộng mang tên
                        An Nông Farm.</p>
                </div>
            </a>
        </div>

        <div class="col-12 col-sm-6 col-md-4 col-xl-3">
            <a href="#" class="news card">
                <img src="<?php echo $imglink['news-tmdt']; ?>" class="news__img">
                <div class="news__body">
                    <h5 class="news__title">Đẩy mạnh tiêu thụ nông sản, hàng Việt lên sàn thương mại điện tử
                    </h5>
                    <p class="news__brief">Bộ Công thương và UBND tỉnh Bình Định tổ chức hội nghị kết nối cung
                        cầu hàng Việt Nam năm 2022 với sự tham dự của các doanh nghiệp, cơ sở sản xuất, phân
                        phối, bán lẻ cùng các sàn thương mại điện tử</p>
                </div>
            </a>
        </div>

        <div class="col-12 col-sm-6 col-md-4 col-xl-3">
            <a href="#" class="news card">
                <img src="<?php echo $imglink['news-xoai']; ?>" class="news__img">
                <div class="news__body">
                    <h5 class="news__title">Sốc với giá xoài cát Hòa Lộc</h5>
                    <p class="news__brief">Giá xoài cát Hòa Lộc đang gây sốc cho thị trường khi tăng thêm từ vài
                        chục tới hơn 150.000 đồng/kg. Đại diện một số HTX ở Đồng Tháp và Tiền Giang cho biết
                        hiện tại sản lượng xoài thấp nên giá có tăng so với mọi năm.</p>
                </div>
            </a>
        </div>
    </div>
</div>